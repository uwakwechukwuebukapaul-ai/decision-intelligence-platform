...
class AIDetectionEngine:
    ...
    def generate(self, threat):
        ...
        return {
            ...
            "rule":
                self.rule_generator.generate(threat),

            "sigma":
                self.sigma_generator.generate(threat),

            "kql":
                self.kql_generator.generate(threat),

            "spl":
                self.spl_generator.generate(threat),

            "coverage":
                self.coverage_analyzer.analyze(threat),

            "false_positive":
                self.false_positive_analyzer.analyze(threat),

            "mitre":
                self.mitre_mapper.map(threat),

            "memory":
                self.memory.store(threat),

            "log":
                self.logger.log(threat),
